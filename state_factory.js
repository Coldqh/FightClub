...
function materializedRosterCount(gameState) {
  var roster = gameState && gameState.rosterState ? gameState.rosterState : null;
  var total = 0;
  var i;
  var fighterId;
  ...
  for (i = 0; i < roster.fighterIds.length; i += 1) {
    fighterId = roster.fighterIds[i];
    if (fighterId && roster.fightersById[fighterId]) {
      total += 1;
    }
  }
  return total;
...
function sparseRosterNeedsRepair(gameState) {
  var roster = gameState && gameState.rosterState ? gameState.rosterState : null;
  if (!roster || !(roster.fighterIds instanceof Array) || !roster.fighterIds.length) {
    return true;
  }
  if (roster.fighterIds.length < 100) {
    return true;
  }
  if (materializedRosterCount(gameState) < 100) {
    return true;
  }
  return false;
}

function repairSparseRosterState(gameState) {
  var repaired = gameState;
  ...
  if (typeof AmateurEcosystem !== "undefined" && AmateurEcosystem.ensureOrganizations) {
    repaired = AmateurEcosystem.ensureOrganizations(repaired) || repaired;
  }
...
function normalizeGameState(gameState, options) {
  ...
  normalized = repairSparseRosterState(normalized);
...
function buildGameStateFromRuntime(runtimeState, options) {
  ...
  gameState = repairSparseRosterState(gameState);
  return gameState;
}
