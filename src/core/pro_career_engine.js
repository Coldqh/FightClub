var ProCareerEngine = (function () {
  ...
  function canTurnPro(gameState, fromTrackId) {
    ...
    var rules = transitionRules();
    if (trackId === "amateur") {
      return age >= (rules.amateurMinAge || 18) &&
        (compareRanks(amateur.rankId || "", rules.amateurMinRankId || "adult_class_2") >= 0 || (amateur.score || 0) >= (rules.amateurMinScore || 140));
    }
    if (trackId === "street") {
      return age >= (rules.streetMinAge || 18) &&
        (street.streetRating || 0) >= (rules.streetMinRating || 100) &&
        ((player.resources && player.resources.fame) || 0) >= (rules.streetMinFame || 15);
    }
    return false;
  }
  ...
  function ensureChampionSeed(gameState, orgTemplate, weekValue) {
    ...
    var firstEntry;
    if (champion && (champion.currentTrack || champion.trackId) === "pro" && champion.status !== "retired") {
      return;
    }
    firstEntry = table && table.entries && table.entries.length ? table.entries[0] : null;
    if (firstEntry && firstEntry.fighterId) {
      assignChampion(gameState, orgTemplate, firstEntry.fighterId, weekValue, champion ? "vacant_fill" : "seeded", "", orgEntity ? orgEntity.championId || "" : "");
    }
  }
